...
public class EGame_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static EGame_MethodManager instance;
	...
	private EGame_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static EGame_MethodManager getInstance() {
		if (instance == null) {
			instance = new EGame_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		clientId = HY_Utils.getManifestMeta(paramActivity, "EGAME_CLIENT_ID");
	...
	public void doLogin(final Activity paramActivity,
		...
		EgameUser.start(paramActivity, Integer.parseInt(clientId),
				loginCallback);
	}

	private CallBackListener loginCallback = new CallBackListener() {

		@Override
		public void onSuccess(String code) {
			mChannelUserInfo.setToken(code);
			EGame_GetUsersInfo egame = new EGame_GetUsersInfo(mActivity);
			egame.getUserInfo(mActivity, mChannelUserInfo,
					new CheckAfter<String>() {
	...
	private String payPrice, paySerialNo;
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		payPrice = String.valueOf(money);
		paySerialNo = mPayParsms.getOrderId();
		...
		EgamePay.pay(mActivity, payParams, new EgamePayListener() {
			...
			public void paySuccess(Map<String, String> params) {
				...
				// dialog.show();
				EgameAgent.onEvent(mActivity, "payment success");
				// 支付成功
			...
			public void payFailed(Map<String, String> params, int errorInt) {
				...
				EgameAgent.onEvent(mActivity, "errorInt", values);
				// 支付失败
			...
			public void payCancel(Map<String, String> params) {
				EgameAgent.onEvent(mActivity, "payment cancelled");
				// 支付取消
	...
	public void doExitQuit(Activity paramActivity,
		...
		EgamePay.exit(paramActivity, new EgameExitListener() {
	...
	public void onActivityResult(int requestCod, int resultCode, Intent data) {
		if (resultCode == ErrorCode.TOEKN_INVALID) {
			// 在此处 重新 调用登陆接口 调用登陆接口 调用登陆接口
			EgameUser.start(mActivity, Integer.parseInt(clientId),
					loginCallback);
		}
		super.onActivityResult(mActivity, requestCod, resultCode, data);
	}
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		EGame_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		EGame_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		EGame_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		EGame_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		EGame_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		EGame_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		EGame_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		EGame_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		EGame_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
