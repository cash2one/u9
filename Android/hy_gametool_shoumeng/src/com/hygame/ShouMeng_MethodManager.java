...
public class ShouMeng_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static ShouMeng_MethodManager instance;
	...
	private ShouMengSDKManager smsdk;

	private ShouMeng_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static ShouMeng_MethodManager getInstance() {
		if (instance == null) {
			instance = new ShouMeng_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		if (mIsLandscape) {
		 ...
		 smsdk = ShouMengSDKManager
				.getInstance(paramActivity);
		SDKInitInfo initInfo = new SDKInitInfo();
		initInfo.setContext(paramActivity);
		initInfo.setDebug(false);
		initInfo.setSdkListener(new OnSDKListener() {
			...
			public void onLogin(CallbackInfo info, int stateCode) {
				if (StateCodes.LOGIN_SUCCESS == stateCode) {
					isLogout = false;
					mChannelUserInfo.setChannelUserId(info.getUserId());
					mChannelUserInfo.setChannelUserName(info.getUserAccount());
					JsonGenerator json = new JsonGenerator();
					json.add("login_account", info.getUserAccount());
					json.add("session_id",info.getTokenInfo() );
					mChannelUserInfo.setToken(json.toString());
		...
		try {  
			json.put("sdkIslandspace", isLandscape);
			} catch (JSONException e) {
			e.printStackTrace();
			}
			initInfo.setExtStr(json.toString());
		smsdk.sdkInit(initInfo);
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		sdkLoginInfo.setActivity(paramActivity);
		smsdk.sdkLogin(sdkLoginInfo);
	}
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		if(mPayParsms.getExchange()==0){
			...
			info.setRate(mPayParsms.getExchange());
		}
		info.setPayType(1);
		info.setProductName(productName);
		info.setRoleId(mChannelUserInfo.getChannelUserId());
		info.setRoleName(ShouMeng_RoleInfo.roleName);
		JSONObject json = new JSONObject();
		try {
		json.put("serverId", ShouMeng_RoleInfo.zoneId);// 服务器ID 必须为数字   
		} catch (JSONException e) {
		}
	...
	public void doExitQuit(Activity paramActivity,
		...
		smsdk.exit(paramActivity, new ExitCallback() {
	...
	public void onResume(Activity paramActivity) {
		if (smsdk!=null){
			smsdk.onSdkResume(paramActivity);
		}
		HyLog.d(TAG, "MethodManager-->onStop");
	}
	...
	public void onPause(Activity paramActivity) {
		if (smsdk!=null){
			smsdk.onSdkPause(paramActivity);
		}
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		ShouMeng_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		ShouMeng_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		ShouMeng_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		ShouMeng_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		ShouMeng_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		ShouMeng_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		ShouMeng_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		ShouMeng_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		ShouMeng_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
