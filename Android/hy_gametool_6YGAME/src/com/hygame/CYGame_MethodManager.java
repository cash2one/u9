...
public class CYGame_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static CYGame_MethodManager instance;
	...
	private YTSDKManager ytsdk;
	private String appkey;

	private CYGame_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static CYGame_MethodManager getInstance() {
		if (instance == null) {
			instance = new CYGame_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		appkey = HY_Utils.getManifestMeta(paramActivity, "MG_APPKEY");
		
		ytsdk = YTSDKManager.getInstance(paramActivity);
		ytsdk.setUserOutListener(new OnUserOutListener() {
	...
	public void doLogin(final Activity paramActivity,
		...
		ytsdk.showLogin(paramActivity, true, new OnLoginListener() {
			
			@Override
			public void loginSuccess(LogincallBack log) {
				ytsdk.showFloatView();
				mChannelUserInfo.setChannelUserId(log.username);
				mChannelUserInfo.setChannelUserName(log.username);
				mChannelUserInfo.setToken(log.sign);
				HyLog.d(TAG, "username:"+log.username);
				HyLog.d(TAG, "sign:"+log.sign);
				HyLog.d(TAG, "logintime:"+log.logintime);
				StringBuffer sign=new StringBuffer();
				 sign.append("username=").append(log.username)
				 .append("&appkey=").append(appkey)
				 .append("&logintime=").append(log.logintime);
				 HyLog.d(TAG, "sign:" + sign);
				 HyLog.d(TAG, "sign:" + HY_Utils.getMD5Hex(sign.toString()));
				 if(HY_Utils.getMD5Hex(sign.toString()).equals(log.sign)){
					 
					 onGotTokenInfo(paramActivity, HY_Constants.DO_LOGIN);
					 
				 }else{
					 mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "登录验证失败");
				 }
			}
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		YTSDKManager.getInstance(paramActivity).showPay(paramActivity,CYGame_RoleInfo.roleId ,money+""
				,CYGame_RoleInfo.zoneId, productName, desc, mPayParsms.getOrderId(),new OnPaymentListener() {
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void onResume(Activity paramActivity) {
		 ytsdk.showFloatView();//显示悬浮窗口
		HyLog.d(TAG, "MethodManager-->onStop");
	}

	@Override
	public void onPause(Activity paramActivity) {
		ytsdk.removeFloatView();
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		CYGame_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		CYGame_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		CYGame_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		CYGame_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		CYGame_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		CYGame_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		CYGame_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		CYGame_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		CYGame_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
