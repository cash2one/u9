...
public class HaiMaWan_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static HaiMaWan_MethodManager instance;
	...
	private HaiMaWan_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static HaiMaWan_MethodManager getInstance() {
		if (instance == null) {
			instance = new HaiMaWan_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		HMPay.init(paramActivity, mIsLandscape, null, new OnCheckUpdateListener() {
		...
		HMPay.setLoginCancelListener(loginCancel);
	}
	OnLoginCancelListener loginCancel = new OnLoginCancelListener() {
	...
	public void doLogin(final Activity paramActivity,
		...
		HMPay.login(paramActivity, logListener);
	}
	OnLoginListener logListener = new OnLoginListener() {
		...
		public void onLoginSuccess(ZHPayUserInfo userInfo) {
			...
			mChannelUserInfo.setToken(userInfo.getLoginToken());
			onGotTokenInfo(mActivity, HY_Constants.DO_LOGIN);
		}
		
		@Override
		public void onLoginFailed(ZHErrorInfo errInfo) {
			mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, errInfo.desc);
		}
...
//		getUserListener().onLogout(HY_SdkResult.SUCCESS, "注销成功");
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		orderInfo.gameName = HY_Utils.getAppName(paramActivity);
		orderInfo.goodName = productName;
		orderInfo.goodPrice = money;
		orderInfo.orderNo = mPayParsms.getOrderId();
		orderInfo.userParam = "sucess";
		
		HMPay.pay(orderInfo, paramActivity,new OnPayListener() {
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void onDestroy(Activity paramActivity) {
		...
		HMPay.removeLoginListener(logListener);
		HMPay.removeLoginCancelListener(loginCancel);
	}
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		HaiMaWan_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		HaiMaWan_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		HaiMaWan_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		HaiMaWan_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		HaiMaWan_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		HaiMaWan_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		HaiMaWan_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		HaiMaWan_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		HaiMaWan_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
