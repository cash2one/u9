        ...
        HY_AccountListener, HY_UserInfoListener
{
    ...
    private void initChannelDate(Activity paramActivity)
    {
        ...
        String merchantId = HY_Utils.getManifestMeta(paramActivity, "DANGLE_SDK_MERCHANTID");
        String appid = HY_Utils.getManifestMeta(paramActivity, "DANGLE_SDK_APPID");
        String appkey = HY_Utils.getManifestMeta(paramActivity, "DANGLE_SDK_APPKEY");
			...
			public void onInitComplete() {
				...
				downjoy.setLogoutListener(new LogoutListener() {
					...
					public void onLogoutSuccess() {
				    	...
				    	isLogout = true;
				        //回调给游戏注销成功
				        getUserListener().onLogout(HY_SdkResult.SUCCESS, "当乐账号注销成功");
					}
                ...
                new CallbackListener<LoginInfo>() {
                    ...
                    public void callback(int status, LoginInfo data) {
                        if (status == CallbackStatus.SUCCESS
                            ...
                            HyLog.d(TAG, mChannelUserInfo.toString());
...
//            mUserInfoTask.startWork(paramActivity, "", this);
    ...
    private void startPayAfter(final Activity paramActivity)
    {
        ...
        float amount = mPayParsms.getAmount();//单位:分
        String productName = mPayParsms.getProductName();
        float money = amount/100;//因为当乐单位是元,换算成元   ，金额 精度问题可以根据渠道需求来
        ...
        String body = "success";//这里一般是跟支付回调一起回调根据需求填写
