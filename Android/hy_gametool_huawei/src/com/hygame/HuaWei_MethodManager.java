...
public class HuaWei_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static HuaWei_MethodManager instance;
	...
	private HuaWei_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static HuaWei_MethodManager getInstance() {
		if (instance == null) {
			instance = new HuaWei_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(final Activity paramActivity) {
		...
		appId = HY_Utils.getManifestMeta(paramActivity, "HUAWEI_APP_ID");
		String cpid = HY_Utils.getManifestMeta(paramActivity, "HUAWEI_CP_ID");
		String privateKey = HY_Utils.getManifestMeta(paramActivity,
				"FLOAT_SECRET_KEY");
		 payId = HY_Utils.getManifestMeta(paramActivity, "HUAWEI_PAY_ID");

		OpenSDK.init(paramActivity, appId, cpid, privateKey, new UserInfo() {
			...
			public void dealUserInfo(HashMap<String, String> userInfo) {
				...
				// 使用华为账号登录且成功，进行accessToken验证
					...
					HyLog.d(TAG, "accesstoken:" + accesstoken);
					mChannelUserInfo.setToken(accesstoken);
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		// 必填字段，不能为null或者""
		payInfo.put("amount", money + ".00");
		// 必填字段，不能为null或者""
		payInfo.put("productName", productName);
		// 必填字段，不能为null或者""
		payInfo.put("requestId", mPayParsms.getOrderId());
		// 必填字段，不能为null或者""
		payInfo.put("productDesc", HY_Utils.getAppName(paramActivity));
		// 必填字段，不能为null或者""，请填写自己的公司名称
		payInfo.put("userName", "深圳优依购互娱科技有限公司");
		// 必填字段，不能为null或者""
		payInfo.put("applicationID", appId);
		// 必填字段，不能为null或者""
		payInfo.put("userID", payId);
		// 必填字段，不能为null或者""
		...
		// 设置支付界面横竖屏，默认竖屏
		payInfo.put("screentOrient", ori);

		IHuaweiPay payHelper = new MobileSecurePayHelper();
		payHelper.startPay(paramActivity, payInfo, new IPayHandler() {
		...
		public void startWork(Activity mActivity, int state, String token,
			...
			if (!isRunning) {
				...
				if (HY_Constants.DO_LOGIN == state
					...
					try {
						json.put("content", getSignContent());
					} catch (JSONException e) {
						e.printStackTrace();
					}
					Map<String, String> map = HttpUtils.getPayInfoRequest(
							mPayParsms, mChannelUserInfo);
					map.put("Ext", java.net.URLEncoder.encode(json.toString()));
				...
				if (null != result && result.obj != null) {
					...
					if (resultVO.transType.equals(TransType.CREATE_USER
						...
						if (resultVO.responseCode.equals("0")) {
							...
							JSONObject json = new JSONObject(resultVO.ext);
							HyLog.d(TAG, "ext:" + resultVO.ext);
							if (!TextUtils.isEmpty(json.getString("userID"))) {
								mChannelUserInfo.setChannelUserId(json
										.getString("userID"));
							}
							if (!TextUtils.isEmpty(json.getString("userName"))) {
								mChannelUserInfo.setChannelUserName(json
										.getString("userName"));
							} else {
								mChannelUserInfo.setChannelUserName(json
										.getString("userID"));
						...
						if (resultVO.responseCode.equals("0")) {
							...
							JSONObject payJson = new JSONObject(resultVO.ext);
							if (payJson.getString("status").equals("0")){
								sign = payJson.getString("msg");
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		HuaWei_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		HuaWei_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		HuaWei_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		HuaWei_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		HuaWei_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		HuaWei_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		HuaWei_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		HuaWei_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		HuaWei_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
		// 这里是为了显示例子,正式的时候就不要弹Toast了
		// Toast.makeText(paramActivity, gameRoleInfo.toString(),
		// Toast.LENGTH_SHORT).show();
		// 存储用户当前的角色信息
		if (HuaWei_RoleInfo.typeId == HY_Constants.ENTER_SERVER) {
			HashMap<String, String> role = new HashMap<String, String>();

			/**
			 * 将用户的等级等信息保存起来，必须的参数为RoleInfo.GAME_RANK(等级)/RoleInfo.GAME_ROLE(
			 * 角色名称) /RoleInfo.GAME_AREA(角色所属区)/RoleInfo.GAME_SOCIATY(角色所属公会名称)
			 * 全部使用String类型存放
			 */
			role.put(RoleInfo.GAME_RANK, HuaWei_RoleInfo.roleLevel);
			role.put(RoleInfo.GAME_ROLE, HuaWei_RoleInfo.roleName);
			role.put(RoleInfo.GAME_AREA, HuaWei_RoleInfo.zoneName);
			role.put(RoleInfo.GAME_SOCIATY, HuaWei_RoleInfo.partyName);
	...
	private String getSignContent() {
		...
		// 必填字段，不能为null或者""
		signInfo.put("amount", mPayParsms.getAmount() / 100 + ".00");
		// 必填字段，不能为null或者""
		signInfo.put("productName", mPayParsms.getProductName());
		//
		signInfo.put("productDesc", HY_Utils.getAppName(mActivity));
		// 必填字段，不能为null或者""
		signInfo.put("requestId", "orderId");
		// 必填字段，不能为null或者""
		signInfo.put("applicationID", appId);
		// 必填字段，不能为null或者""
		signInfo.put("userID", payId);
		String signContent = HuaweiPayUtil.getSignData(signInfo);
		HyLog.d(TAG, "signContent:" + signContent);
		return signContent;
	}
