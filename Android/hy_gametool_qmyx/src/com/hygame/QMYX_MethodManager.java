...
import com.haobangnet.gamesdk.view.activity.QMYXActivity;
...
public class QMYX_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static QMYX_MethodManager instance;
	...
	private QMYX_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static QMYX_MethodManager getInstance() {
		if (instance == null) {
			instance = new QMYX_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		try {
			QMYXSDK.initSDK(paramActivity, false, QMYXLogLevel.WARN, new QMYXCallbackListener<String>() {
	...
	public void doLogin(final Activity paramActivity,
		...
		try {
			QMYXSDK.login(paramActivity, new QMYXCallbackListener<String>() {
				
				@Override
				public void callback(int code, String msg) {
					switch (code) {
					case QMYXGameSDKStatusCode.SUCCESS:
						mChannelUserInfo.setToken(QMYXSDK.getSessionId());
						QMYX_GetUserInfo userInfo = new QMYX_GetUserInfo(paramActivity);
						userInfo.getUserInfo(mChannelUserInfo, new CheckAfter<String>() {
							...
							public void afterFailed(String paramString, Exception paramException) {
								ToastUtils.show(paramActivity, "登录验证失败");
							}
						});
						break;
					...
					case QMYXGameSDKStatusCode.NO_INIT:
						mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "登录失败:未初始化");
						break;
					default:
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		QMYXPaymentInfo payInfo = new QMYXPaymentInfo();
		payInfo.setAmount(money);
		payInfo.setServerId(Integer.valueOf(QMYX_RoleInfo.zoneId));
		payInfo.setPayDesc(productName);
		payInfo.setCustomInfo(mPayParsms.getOrderId());
		
		try {
			QMYXSDK.pay(paramActivity, payInfo, new QMYXCallbackListener<OrderInfo>() {
				
				@Override
				public void callback(int code, OrderInfo msg) {
					switch (code) {
					case QMYXGameSDKStatusCode.NO_INIT:
							mPayCallBack.onPayCallback(HY_SdkResult.FAIL,"游戏未初始化");
						break;
					case QMYXGameSDKStatusCode.SUCCESS:
					...
					case QMYXGameSDKStatusCode.FAIL:
						...
						break;
					case QMYXGameSDKStatusCode.NO_LOGIN:
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL, "游戏未登录");
						break;
					}
				}
			});
...
//		mExitCallback.onGameExit();
		try {
			QMYXSDK.exitSDK(new QMYXCallbackListener<String>() {
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		QMYX_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		QMYX_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		QMYX_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		QMYX_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		QMYX_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		QMYX_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		QMYX_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		QMYX_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		QMYX_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
