...
public class C07073_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static C07073_MethodManager instance;
	...
	private C07073_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static C07073_MethodManager getInstance() {
		if (instance == null) {
			instance = new C07073_MethodManager();
		}
		return instance;
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		SY07073API.getInstance().login(mActivity, new SDKCallBack(){
			@Override
			public void callback(String s) {
				try{
	        		...
	        		if(state == 1){ //成功
	        			...
	        			HyLog.e(TAG, "onLoginFailed: state:" + state + "msg:" + msg);
	        		}
				} catch (JSONException e) {
					e.printStackTrace();
					mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "登录失败");
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void onResume(Activity paramActivity) {
		HyLog.d(TAG, "MethodManager-->onStop");
		FloatWindow.getInstance(mActivity.getApplicationContext()).show();
	}

	@Override
	public void onPause(Activity paramActivity) {
		FloatWindow.getInstance(mActivity.getApplicationContext()).hide();	
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		C07073_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		C07073_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		C07073_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		C07073_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		C07073_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		C07073_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		C07073_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		C07073_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		C07073_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
