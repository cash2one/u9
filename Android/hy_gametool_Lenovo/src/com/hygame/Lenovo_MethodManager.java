...
public class Lenovo_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static Lenovo_MethodManager instance;
	...
	private Lenovo_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static Lenovo_MethodManager getInstance() {
		if (instance == null) {
			instance = new Lenovo_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		appid = HY_Utils.getManifestMeta(paramActivity, "lenovo.open.appid");
		appkey = HY_Utils.getManifestMeta(paramActivity, "lenovo.open.appkey");
		LenovoGameApi.doInit(paramActivity, appid);
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		LenovoGameApi.doAutoLogin(paramActivity, new IAuthResult() {

			@Override
			public void onFinished(boolean ret, String data) {
				if (ret) {
					// 登录成功
					mChannelUserInfo.setToken(data);
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		String waresid = "";
		waresid = HY_Utils.getPayInfoId(paramActivity,
				dataFromAssets.getmReservedParam1(), money / 100);
		// 请填写商品自己的参数
		...
		payRequest.addParam("exorderno", mPayParsms.getOrderId());
		payRequest.addParam("price", money);
		payRequest.addParam("cpprivateinfo", "ok");
		LenovoGameApi.doPay(paramActivity, appkey, payRequest,
				new IPayResult() {
	...
	public void doExitQuit(Activity paramActivity,
		...
		LenovoGameApi.doQuit(paramActivity, new IQuitCallback() {
				...
				if (null != result && result.obj != null) {
					...
					if (resultVO.transType.equals(TransType.CREATE_USER
						...
						if (resultVO.responseCode.equals("0")) {
							...
							if (!TextUtils.isEmpty(json.getString("AccountID"))) {
								mChannelUserInfo.setChannelUserId(json.getString("AccountID"));
							}
							if (!TextUtils.isEmpty(json.getString("Username"))) {
								mChannelUserInfo.setChannelUserName(json.getString("Username"));
							}
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		Lenovo_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		Lenovo_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		Lenovo_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		Lenovo_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		Lenovo_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		Lenovo_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		Lenovo_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		Lenovo_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		Lenovo_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
