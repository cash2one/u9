...
public class OPPO_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY_OPPO_SDK";
	private static OPPO_MethodManager instance;
	...
	private OPPO_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static OPPO_MethodManager getInstance() {
		if (instance == null) {
			instance = new OPPO_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		if (mIsLandscape) {
		 ...
		 appKey = HY_Utils.getManifestMeta(paramActivity, "OPPO_APPKEY");
		 appSecret = HY_Utils.getManifestMeta(paramActivity,"OPPO_APPSECRET");

		GameCenterSettings gameCenterSettings = new GameCenterSettings(false, // 网游固定为false
				appKey, appSecret, false, // 调试开关 true 打印log，false
											// 关闭log，正式上线请设置为false
				mIsLandscape); // 将游戏横竖屏状态传递给sdk， true为竖屏 false为横屏
		GameCenterSDK.init(gameCenterSettings, mActivity);
	...
	public void doLogin(final Activity paramActivity,
		...
		GameCenterSDK.getInstance().doLogin(mActivity, new ApiCallback() {
			...
			public void onFailure(String resultMsg, int resultCode) {
				...
				switch (resultCode) {
				case 8000:
					loginCallBack.onLoginFailed(HY_SdkResult.CANCEL,resultMsg +resultCode);
					break;
	...
	private void doGetUserInfoByCpClient(String token, String ssoid) {
		GameCenterSDK.getInstance().doGetUserInfo(new ReqUserInfoParam(token, ssoid), new ApiCallback() {
					...
					public void onSuccess(String resultMsg) {
						...
						HyLog.e(TAG, "[userinfo]" + resultMsg);
						// {"userName":"U42376666","email":"","mobile":"159******22","ssoid":"125449948"}
						...
						try {
							JSONObject json = new JSONObject(resultMsg);
							Log.e(TAG, "ssoid:" + (String) json.get("ssoid"));
							Log.e(TAG,
									"userName:" + (String) json.get("userName"));
							// 正常接入
							mChannelUserInfo.setChannelUserId((String) json
									.get("ssoid"));// 渠道uid
							mChannelUserInfo.setChannelUserName((String) json
									.get("userName"));// 渠道用户名
					...
					@Override
					public void onFailure(String resultMsg, int resultCode) {
						HyLog.d(TAG, "用户信息获取失败");
						mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "登录失败:用户信息获取失败");
					}
				});
	}
	...
	public void doStartPay(Activity paramActivity, HY_PayParams payParams,
		...
		if (null == mChannelUserInfo) {
			...
			HyLog.e(TAG, "doStartPay:" + mChannelUserInfo.getUserId());
			// 这里是网络请求方法
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		int amount = mPayParsms.getAmount();// 单位:分 // 支付金额，单位分

		PayInfo payInfo = new PayInfo(orderId, "", amount);
		payInfo.setProductDesc(exchange * amount / 100
				+ mPayParsms.getProductName());
		payInfo.setProductName(mPayParsms.getProductName());
		payInfo.setAttach("success");
		HyLog.d(TAG, "url:"+dataFromAssets.getChannelCallbackUrl());
		payInfo.setCallbackUrl(dataFromAssets.getChannelCallbackUrl());
		GameCenterSDK.getInstance().doPay(mActivity, payInfo,
				new ApiCallback() {

					@Override
					public void onSuccess(String resultMsg) {
						HyLog.e(TAG, "支付成功:" + resultMsg);
					...
					public void onFailure(String resultMsg, int resultCode) {
						if (PayResponse.CODE_CANCEL != resultCode) {
							HyLog.e(TAG, "支付失败:" + resultMsg);
							mPayCallBack.onPayCallback(HY_SdkResult.FAIL,
									"支付失败:" + resultMsg);
						} else {
							HyLog.e(TAG, "支付取消:" + resultMsg);
	...
	public void onGotTokenInfo(Activity paramActivity, int state) {
		...
		mProgress = ProgressUtil.showByString(mActivity, "登录验证信息",
				"正在请求V5应用服务器，请稍候……", new DialogInterface.OnCancelListener() {
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		OPPO_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
														// 登录:HY_Constants.ENTER_SERVER
														// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		OPPO_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		OPPO_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		OPPO_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		OPPO_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		OPPO_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		OPPO_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		OPPO_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		OPPO_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
		...
		GameCenterSDK.getInstance().doReportUserGameInfoData(
				new ReportUserGameInfoParam(OPPO_RoleInfo.roleId,
						OPPO_RoleInfo.zoneName, OPPO_RoleInfo.roleName,
						OPPO_RoleInfo.roleLevel), new ApiCallback() {

					@Override
					public void onSuccess(String resultMsg) {
						HyLog.d(TAG, "角色信息上传成功");
					}

					@Override
					public void onFailure(String resultMsg, int resultCode) {
						HyLog.e(TAG, "角色信息上传失败");
					}
				});
