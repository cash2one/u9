...
public class SanguoCardMain extends Activity implements OnClickListener
{
 ...
 **/
        ...
        HY_GameProxy.getInstance().setUserListener(this,new HY_UserListener() {
			...
			public void onSwitchUser(HY_User user,int resultCode){
				...
				case HY_SdkResult.SUCCESS:
					...
					//如果切换统一角色,默认回传切换取消
					 ...
					 Toast.makeText(SanguoCardMain.this, "切换账号成功", Toast.LENGTH_SHORT).show();
					break;
				case HY_SdkResult.CANCEL:
					//切换账号取消不做任何处理
					 HyLog.d(TAG, "切换账号取消");
					 Toast.makeText(SanguoCardMain.this, "切换账号取消", Toast.LENGTH_SHORT).show();
					break;
				default:
					//切换账号失败不做任何处理
					 HyLog.d(TAG, "切换账号失败");
					 Toast.makeText(SanguoCardMain.this, "切换账号失败", Toast.LENGTH_SHORT).show();
					break;
				}
			...
			public void onLogout(int resultCode,Object paramObject) {
				 ...
				 case HY_SdkResult.SUCCESS  :
				        ...
				        HyLog.d(TAG,"注销成功");
				        Toast.makeText(SanguoCardMain.this, "注销成功", Toast.LENGTH_LONG).show();
				        break;

				default:
						Toast.makeText(SanguoCardMain.this, "注销失败", Toast.LENGTH_LONG).show();
						HyLog.d(TAG,"注销失败");
						break;
				 }
			}
    ...
    private void doLogOn()
    {
        ...
        HY_GameProxy.getInstance().logon(SanguoCardMain.this, new HY_LoginCallBack() {
			
			 @Override
		        public void onLoginSuccess(HY_User user)
		        {
		            	//登录
			            Toast.makeText(SanguoCardMain.this, "登录成功", Toast.LENGTH_LONG).show();
			            mUser = user;
			            mUserInfoTv.setText("userId: " + mUser.getUserId() + "\n渠道用户id:"
			                    + mUser.getChannelUserId() + "\n渠道用户名: " + mUser.getChannelUserName()
			                    + "\ntoken: " + mUser.getToken());
		        ...
		        public void onLoginFailed(int code ,String message)
		        {
					...
					case HY_SdkResult.CANCEL:
						//取消登录
						Toast.makeText(SanguoCardMain.this, message, Toast.LENGTH_LONG).show();
						HyLog.d(TAG,"取消登录");
						break;
    ...
    private void doPay()
    {
        ...
        HY_GameProxy.getInstance().startPay(SanguoCardMain.this, payParams, new HY_PayCallBack()
        {
            @Override
            public void onPayCallback(int retCode, String paramString)
            {
               ...
               switch (retCode) {
			case 0:
				 Toast.makeText(SanguoCardMain.this, "支付成功", Toast.LENGTH_LONG).show();
				break;
			case 2:
				 Toast.makeText(SanguoCardMain.this, "支付取消", Toast.LENGTH_LONG).show();
				break;
			case 3:
				Toast.makeText(SanguoCardMain.this, "支付进行中", Toast.LENGTH_LONG).show();
				break;
			default:
				Toast.makeText(SanguoCardMain.this, "支付失败", Toast.LENGTH_LONG).show();
				break;
               }
            }
        });
    ...
    {
        HY_GameProxy.getInstance().logoff(SanguoCardMain.this, "注销");
    }
        ...
        HY_GameProxy.getInstance().exit(this, new HY_ExitCallback()
        {
            ...
            public void onGameExit()
            {
               ...
               //退出确认窗口需要游戏自定义，并且实现资源回收，杀死进程等退出逻辑 
                AlertDialog.Builder builder = new Builder(SanguoCardMain.this);
            ...
            public void onChannelExit()
            {
              ...
              //退出确认窗口由渠道提供，游戏只须实现资源回收，杀死进程等退出逻辑
                Toast.makeText(SanguoCardMain.this, "由渠道退出界面退出",
                        Toast.LENGTH_LONG).show();
                HY_GameProxy.getInstance()
                        .applicationDestroy(SanguoCardMain.this);
                SanguoCardMain.this.finish();
    ...
    protected void doCheckLogin()
    {
        if (mUser == null)
        {
            Toast.makeText(SanguoCardMain.this, "请先登陆", Toast.LENGTH_LONG).show();
            return;
        }
        HY_CheckReLogin reLoginChecker=new HY_CheckReLogin(SanguoCardMain.this);
        reLoginChecker.checkLogin(mUser, new CheckAfter<String>() {
             ...
             {
             	doSetRoleData();
                 ToastUtils.show(SanguoCardMain.this, "验证登陆成功");
             }
             
             @Override
             public void afterFailed(String paramString, Exception paramException)
             {
                 ToastUtils.show(SanguoCardMain.this, "验证登陆失败");
