...
public class Xcs_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static Xcs_MethodManager instance;
	...
	private Xcs_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static Xcs_MethodManager getInstance() {
		if (instance == null) {
			instance = new Xcs_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		mAppId = HY_Utils.getManifestMeta(paramActivity, "APPID");
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		XcsSDK.getInstance(mActivity).login(mAppId, new LoginListener() {
			@Override
			public void LoginRequest(Bundle request) {
				String code = request.getString("code");// 状态码

				if (code == "0000") { // 登录成功
					isLogout = false;
					String u_id = request.getString("u_id");// 用户ID
					String sign = request.getString("sign");// 用户验证签名

					mChannelUserInfo.setChannelUserId(u_id);
					mChannelUserInfo.setToken(sign);
					mChannelUserInfo.setChannelUserName(u_id);
					onGotTokenInfo(mActivity, HY_Constants.DO_LOGIN);
				} else if (code == "0001") { // 网络连接失败
					mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "网络连接失败");
				} else if (code == "0002") { // 参数错误
					mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "参数错误");
				} else if (code == "0003") { // 未知异常
					mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "未知异常");
				} else if (code == "0004") { // 未登录
					mLoginCallBack.onLoginFailed(HY_SdkResult.FAIL, "未登录");
				}
			}
		});
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		Bundle b = new Bundle();
		HyLog.d(TAG, "支付回调地址:"+dataFromAssets.getChannelCallbackUrl());
		b.putString("notify_uri", dataFromAssets.getChannelCallbackUrl());// 回调地址
		b.putString("server_name", Xcs_RoleInfo.zoneName);// 区服名称
		b.putString("character", Xcs_RoleInfo.roleName);// 角色名称
		b.putString("game_name", HY_Utils.getAppName(paramActivity));// 游戏名称
		b.putString("game_uid", Xcs_RoleInfo.roleId);// 游戏服务器用户id
		b.putString("order_id", mPayParsms.getOrderId());// 订单号
		b.putString("product_id", mPayParsms.getProductId());// 商品id
		b.putInt("Price", mPayParsms.getAmount() / 100);// （单位为元）
		b.putString("order_sign", mSign);

		XcsSDK.getInstance(mActivity).pay(b, new PayListener() {
			@Override
			public void Request(Bundle request) {
				if (request != null) {
					...
					if (code == "0000") {
						/*
						request.getString("notify_uri");// 回调地址
						request.getString("server_name");// 区服名称
						request.getString("character");// 角色名称
						request.getString("game_name");// 游戏名称
						request.getString("game_uid");// 游戏服务器用户id
						request.getString("order_id");// 订单号
						request.getString("product_id");// 物品id
						request.getInt("Price");// 价格（单位为元）
						*/						
						mPayCallBack.onPayCallback(HY_SdkResult.SUCCESS, "支付成功");
					} else if (code == "0001") { // 网络连接失败
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL, "网络连接失败");
					} else if (code == "0002") { // 参数错误
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL, "参数错误");
					} else if (code == "0003") { // 未知异常
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL, "未知异常");
					} else if (code == "0004") { // 未支付
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL, "未支付");
					} else if (code == "0005") { // 当前手机登陆授权失效，账号已在其他手机登陆
						mPayCallBack.onPayCallback(HY_SdkResult.FAIL,"当前手机登陆授权失效，账号已在其他手机登陆");
					}
		...
		public void startWork(Activity mActivity, int state, String token,
			...
			if (!isRunning) {
				...
				if (HY_Constants.DO_LOGIN == state
					...
					try {
						json.put("product_id", mPayParsms.getProductId());
						json.put("price", mPayParsms.getAmount() / 100);
						json.put("game_uid", Xcs_RoleInfo.roleId);
						json.put("u_id", mChannelUserInfo.getChannelUserId());
						json.put("game_id", mAppId);
	...
	public void onDestroy(Activity paramActivity) {
		XcsSDK.getInstance(paramActivity).onDestroy();
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		Xcs_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
														// 登录:HY_Constants.ENTER_SERVER
														// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		Xcs_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		Xcs_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		Xcs_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		Xcs_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		Xcs_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		Xcs_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		Xcs_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		Xcs_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
