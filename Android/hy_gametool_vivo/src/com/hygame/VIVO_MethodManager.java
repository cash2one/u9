...
public class VIVO_MethodManager extends HY_UserManagerBase implements
	...
	private void initChannelDate(Activity paramActivity) {
		...
		appid = HY_Utils.getManifestMeta(paramActivity, "APP_MONITOR_APPID");
		String gameId = HY_Utils.getManifestMeta(paramActivity, "HY_GAME_ID");
		String channleCode = HY_Utils.getManifestMeta(paramActivity, "HY_CHANNEL_CODE");
		gameName = HY_Utils.getAppName(paramActivity);
		if(!TextUtils.isEmpty(dataFromAssets.getChannelCallbackUrl())){
			notifyUrl = dataFromAssets.getChannelCallbackUrl();
		}else{
			notifyUrl = "http://115.159.73.234/api/channelPayNotify/" + channleCode + "/" + gameId;
		}
		HyLog.d(TAG, "notifyUrl:"+notifyUrl);
		mVivoUnionManager = new VivoUnionManager(paramActivity);
		mVivoUnionManager.registVivoAccountChangeListener (accountListener);
		mVivoUnionManager.initVivoPaymentAndRecharge(paramActivity, payResultListener);
	...
	OnVivoAccountChangedListener accountListener = new OnVivoAccountChangedListener() {
		...
		public void onAccountLogin(String name, String openid, String authtoken) {
			HyLog.d(TAG, "isLogout:"+isLogout);
			if(isLogout != false){
				mChannelUserInfo.setChannelUserId(openid);
				mChannelUserInfo.setChannelUserName(name);
				mChannelUserInfo.setToken(authtoken);
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		localBundle.putString("transNo", orderNumber);
		localBundle.putString("accessKey", accessKey);
		localBundle.putString("productName", productName);
		localBundle.putString("productDes", gameName);
		localBundle.putLong("price", money);
		localBundle.putString("appId", appid);
		
		HyLog.d(TAG, "transNo:"+mPayParsms.getOrderId());
		HyLog.d(TAG, "accessKey:"+accessKey);
		HyLog.d(TAG, "productName:"+productName);
		HyLog.d(TAG, "productDes:"+gameName);
		HyLog.d(TAG, "appId:"+appid);
		HyLog.d(TAG, "extInfo:"+ money+"");
//		localBundle.putString("blance", VIVO_RoleInfo.balance+"");
//		localBundle.putString("vip", VIVO_RoleInfo.vip);
//		localBundle.putInt("level", Integer.valueOf(VIVO_RoleInfo.roleLevel));
//		localBundle.putString("party", VIVO_RoleInfo.partyName);
//		localBundle.putString("roleId", VIVO_RoleInfo.roleId);
//		localBundle.putString("roleName", VIVO_RoleInfo.roleName);
//		localBundle.putString("serverName", VIVO_RoleInfo.zoneName);
		localBundle.putString("extInfo", money+"");
		localBundle.putBoolean("logOnOff", true); 
		
		mVivoUnionManager.payment(paramActivity, localBundle);
	}
			...
			if (!isRunning) {
				...
				if (HY_Constants.DO_LOGIN == state
					...
					try {
						...
						json.put("orderAmount", mPayParsms.getAmount()+"");
						json.put("orderTitle", mPayParsms.getProductName());
						json.put("orderDesc", gameName);
						json.put("extInfo", mPayParsms.getAmount()+"");
						HyLog.d(TAG, "version:"+version);
						HyLog.d(TAG, "notifyUrl:"+notifyUrl);
						HyLog.d(TAG, "orderAmount:"+mPayParsms.getAmount());
						HyLog.d(TAG, "orderTitle:"+ mPayParsms.getProductName());
						HyLog.d(TAG, "orderDesc:"+gameName);
						HyLog.d(TAG, "extInfo:"+mPayParsms.getAmount());
				...
				if (null != result && result.obj != null) {
					...
					if (resultVO.transType.equals(TransType.CREATE_USER
						...
						if (resultVO.responseCode.equals("0")) {
							...
							HyLog.d(TAG, "Ext:"+resultVO.ext);
							JSONObject resultJson = new JSONObject(resultVO.ext);
							accessKey = resultJson.getString("accessKey");
							orderNumber = resultJson.getString("orderNumber");
							HyLog.d(TAG, "accessKey:"+accessKey);
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void onDestroy(Activity paramActivity) {
		...
		mVivoUnionManager
		.unRegistVivoAccountChangeListener(accountListener);
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		VIVO_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		VIVO_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		VIVO_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		VIVO_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		VIVO_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		VIVO_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		VIVO_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		VIVO_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		VIVO_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
		// 这里是为了显示例子,正式的时候就不要弹Toast了
		// Toast.makeText(paramActivity, gameRoleInfo.toString(),
		// Toast.LENGTH_SHORT).show();
		HyLog.d(TAG, "MethodManager-->setExtData");
		
		if(VIVO_RoleInfo.typeId == HY_Constants.ENTER_SERVER){
			VivoUnionManager.vivoAccountreportRoleInfo(VIVO_RoleInfo.roleId, VIVO_RoleInfo.roleLevel,
					VIVO_RoleInfo.zoneId, VIVO_RoleInfo.roleName, paramActivity, VIVO_RoleInfo.zoneName);
		}
	}
