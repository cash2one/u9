...
public class PYW_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static PYW_MethodManager instance;
	...
	private PYW_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static PYW_MethodManager getInstance() {
		if (instance == null) {
			instance = new PYW_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		if (mIsLandscape) {
			// 这里是横屏，该渠道没有横竖屏配置,忽略
			orientationInfo = OrientationInfo.LANDSCAPE;
			HyLog.d(TAG, "这里是横屏");
		} else {
			// 如果有,就通过这个判断来设置相关
			orientationInfo = OrientationInfo.PORTRAIT;
			HyLog.d(TAG, "这里是竖屏");
		}
		String gameKey = HY_Utils.getManifestMeta(paramActivity, "GAME_KEY");
		SDKConfig config = new SDKConfig();
		config.setGameKey(gameKey);
		config.setActivityOrientation(orientationInfo);
		PYWPlatform.initSDK(paramActivity, config, new OnSDKEventListener() {
			...
			public void onEvent(int eventCode, Bundle data) {
				...
				case ISDKEventCode.CODE_LOGIN_SUCCESS:
					...
					if (data != null) {
						...
						if (user != null) {
							...
							// 朋友玩为用户分配的唯一标识
							isLogout = false;
							mChannelUserInfo.setChannelUserId(user.getUserId());
				...
				case ISDKEventCode.CODE_BACK_TO_GAME:
					// 从sdk界面返回到游戏界面
					User userinfo = PYWPlatform.getCurrentUser();
					if (userinfo != null) {
						// Toast.makeText(mContext, "用户:" +
						// userinfo.getUserName() + "从sdk界面返回至游戏",
						...
						mLoginCallBack.onLoginFailed(HY_SdkResult.CANCEL,
								"取消登录");
						// Toast.makeText(mContext, "从sdk界面返回至游戏(未登录)",
						// Toast.LENGTH_LONG).show();
					}
					break;
				case ISDKEventCode.CODE_LOGIN_FAILD:
					String erroMsg = data
							.getString(ISDKEventExtraKey.EXTRA_ERRO);
					// Log.d("SDKCallback", erroMsg);
					break;
				case ISDKEventCode.CODE_CHARGE_SUCCESS:
					// 充值成功
					// Log.d("SDKCallback", "充值成功:" +
				...
				case ISDKEventCode.CODE_CHANGE_ACCOUNT_SUCCESS:
					...
					if (data != null) {
						...
						if (user != null) {
							isLogout = false;
							mChannelUserInfo.setChannelUserId(user.getUserId());
							mChannelUserInfo.setChannelUserName(user.getToken());
							mChannelUserInfo.setToken(user.getToken());
							onGotTokenInfo(mActivity,
	...
	public void doLogin(final Activity paramActivity,
		...
		PYWPlatform.openLogin(paramActivity);
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		String payId  = "";
		float hyMoney = 1;
		float number1 = mPayParsms.getExchange() * money;
		float number2 = 10 * money;
		long gameCurrency = mPayParsms.getExchange() != 0 ? (long)number1 : (long)number2;
		//String productName = mPayParsms.getProductName();
		//String desc = money * mPayParsms.getExchange() + productName;
		try{
			HY_GamePayConfig payConf = HY_GamePayConfig.getInstance(paramActivity);
			JSONArray arr = payConf.getKey(paramActivity);
			for (int i=0;i<arr.length();i++){
				try {
					hyMoney = Integer.valueOf(arr.get(i).toString());
					if (Float.valueOf(arr.get(i).toString())==money){
						payId = payConf.get(arr.get(i).toString());
						break;
					}
		...
		if(hyMoney == money){
			String jsonParam = getOrderParams(Integer.valueOf(payId)); 
			PYWPlatform.openChargeCenter(paramActivity, money, gameCurrency, jsonParam);
		}else{
			ToastUtils.show(paramActivity, "该支付金额不存在");
		}
	}

    // 模拟厂商需要发送的参数项
    private String getOrderParams(int productId) {
        JSONObject jobj = new JSONObject();
        try {
            // 订单id，此项必须要填写，并且参数名必须为"order_id"，否则会出错
            jobj.put("order_id", "" + mPayParsms.getOrderId());
            // 产品id，此项必须要填写，并且参数名必须为"product_id"，否则会出错(提示:商品id需要是整数值)
            // ：如果是任意充可以不填
            jobj.put("product_id", productId); // 6660
...
//            jobj.put("channel", "渠道号");
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		PYW_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
														// 登录:HY_Constants.ENTER_SERVER
														// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		PYW_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		PYW_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		PYW_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		PYW_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		PYW_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		PYW_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		PYW_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		PYW_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
