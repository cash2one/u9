...
public class Mgw_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static Mgw_MethodManager instance;
	...
	private OnBNSDKListener mSdkListener;
	
	private Mgw_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static Mgw_MethodManager getInstance() {
		if (instance == null) {
			instance = new Mgw_MethodManager();
		}
		return instance;
	}
	 ...
	 */
	@Override
	public void applicationInit(Activity paramActivity, boolean isLandscape) {
		...
		mSdkListener = new OnBNSDKListener() {
			
			@Override
			public void OnOrderFinish(int arg0, BNSDKOrderResult arg1) {
				HyLog.d(TAG, "BNSDKOrderResult:" + arg1);
				mPayCallBack.onPayCallback(HY_SdkResult.SUCCESS, "支付成功");
			}
			
			@Override
			public void OnOrderCancel(String arg0) {
				HyLog.d(TAG, "OnOrderCancel:" + arg0);
//				mPayCallBack.onPayCallback(HY_SdkResult.CANCEL,"取消支付");
			}
			
			@Override
			public void OnOrderApply(String arg0) {
				HyLog.d(TAG, "OnOrderApply:" + arg0);
			}
...
//				mLoginCallBack.onLoginFailed(HY_SdkResult.CANCEL, "登录取消");
	...
	public void doLogin(final Activity paramActivity,
		...
		BNSDK.getInstance().login(mActivity, mSdkListener);
	...
	private void startPayAfter(final Activity paramActivity) {
        ...
        BNSDK.getInstance().order(mActivity, null, productName, money, orderId, mSdkListener);
	}
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		Mgw_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		Mgw_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		Mgw_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		Mgw_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		Mgw_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		Mgw_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		Mgw_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		Mgw_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		Mgw_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
