...
public class AnFeng_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static AnFeng_MethodManager instance;
	...
	private AnFeng_MethodManager() {
		mChannelUserInfo = new HY_UserInfoVo();
	}

	public static AnFeng_MethodManager getInstance() {
		if (instance == null) {
			instance = new AnFeng_MethodManager();
		}
		return instance;
	}
	...
	private void initChannelDate(Activity paramActivity) {
		...
		String appkey = HY_Utils
				.getManifestMeta(paramActivity, "ANFENG_APPKEY");
		sdk.checkGameUpdate(paramActivity);

		CPInfo info = new CPInfo(appkey);
		sdk.init(paramActivity, info);
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		sdk.anfanLogin(paramActivity, new LoginCallback() {
			/**
			 * @param uid
			 *            用户登录名
			 * @param uuid
			 ...
			 */
			@Override
			public void onLoginSuccess(String uid, String uuid, String ucid) {
				isLogout = false;
				mChannelUserInfo.setChannelUserId(ucid);
				mChannelUserInfo.setChannelUserName(uid);
				mChannelUserInfo.setToken(uuid);
				sdk.addFloatBall(paramActivity);
	...
	public void doLogout(final Activity paramActivity, Object object) {
		...
		sdk.removeFloatball(paramActivity);
	}
	...
	private void startPayAfter(final Activity paramActivity) {
		...
		OrderInfo info = new OrderInfo(mPayParsms.getOrderId(),money+".00", productName, productName);
		sdk.anFanPay(paramActivity,info , dataFromAssets.getChannelCallbackUrl(), new PayCallback() {
			
			@Override
			public void onPaySuccess(String arg0) {
				mPayCallBack.onPayCallback(HY_SdkResult.SUCCESS, "支付成功");
			}
			
			@Override
			public void onPayFailure(String arg0) {
	...
	public void doExitQuit(Activity paramActivity,
		...
		sdk.anfanLogout(paramActivity, new LogoutCallback() {
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		AnFeng_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		AnFeng_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		AnFeng_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		AnFeng_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		AnFeng_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		AnFeng_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		AnFeng_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		AnFeng_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		AnFeng_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
		// 这里是为了显示例子,正式的时候就不要弹Toast了
		// Toast.makeText(paramActivity, gameRoleInfo.toString(),
		// Toast.LENGTH_SHORT).show();
		if (AnFeng_RoleInfo.typeId == HY_Constants.ENTER_SERVER) {
			sdk.setRoleData(paramActivity, AnFeng_RoleInfo.roleId,
					AnFeng_RoleInfo.roleName, AnFeng_RoleInfo.roleLevel,
					AnFeng_RoleInfo.zoneId, AnFeng_RoleInfo.zoneName);
		}
