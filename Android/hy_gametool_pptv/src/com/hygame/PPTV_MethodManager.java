...
public class PPTV_MethodManager extends HY_UserManagerBase implements
		HY_AccountListener, HY_UserInfoListener {
	private static final String TAG = "HY";
	private static PPTV_MethodManager instance;
	...
	public static PPTV_MethodManager getInstance() {
		if (instance == null) {
			instance = new PPTV_MethodManager();
		}
		return instance;
	}
	...
	public void onCreate(Activity paramActivity) {
		mActivity = paramActivity;
		PptvVasAgent.showFloatingView(paramActivity);
	}
	...
	public void doLogin(final Activity paramActivity,
		...
		PptvVasAgent.startLoginActivity(mActivity, mLoginListener);
	...
	private void startPayAfter(final Activity paramActivity) {
        ...
        String zoneId = "1";
        if (!TextUtils.isEmpty(PPTV_RoleInfo.zoneId) || (PPTV_RoleInfo.zoneId != "0")){
        	zoneId = PPTV_RoleInfo.zoneId;
        }
        
        String orderId = mPayParsms.getOrderId(); 
        PptvVasAgent.startPayActivity(mActivity, zoneId, PPTV_RoleInfo.roleId, orderId,
        		1, mPayParsms.getAmount() / 100 + "" , mPayParsms.getProductName(), mPayListener);
	}
...
//		super.onActivityResult(paramActivity, requestCode, resultCode, data);
	...
	public void setRoleData(Activity paramActivity, HY_GameRoleInfo gameRoleInfo) {
		...
		PPTV_RoleInfo.typeId = gameRoleInfo.getTypeId();// 上传类型
															// 登录:HY_Constants.ENTER_SERVER
															// 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
		PPTV_RoleInfo.roleId = gameRoleInfo.getRoleId();// 角色id
		PPTV_RoleInfo.roleName = gameRoleInfo.getRoleName();// 角色名
		PPTV_RoleInfo.roleLevel = gameRoleInfo.getRoleLevel();// 角色等级
		PPTV_RoleInfo.zoneId = gameRoleInfo.getZoneId();// 区服id
		PPTV_RoleInfo.zoneName = gameRoleInfo.getZoneName();// 区服名
		PPTV_RoleInfo.balance = gameRoleInfo.getBalance(); // 用户余额
		PPTV_RoleInfo.vip = gameRoleInfo.getVip();// vip等级
		PPTV_RoleInfo.partyName = gameRoleInfo.getPartyName();// 帮派名称
