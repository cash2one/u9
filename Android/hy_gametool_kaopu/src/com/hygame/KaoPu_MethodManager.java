    ...
    private void initChannelDate(Activity paramActivity)
    {
        ...
        KPSuperSDK.setGetCheckUrlCallBack(new KPGetCheckUrlCallBack() {
            ...
            public void onGetCheckUrlSuccess(String url) {
            	HyLog.d(TAG, "登录验证URL:"+url);
            }

            @Override
            public void onGetCheckUrlFailed() {
            	HyLog.e(TAG, "登录验证URL获取失败");
            }
        });
        ...
        configData.put("gameName",HY_Utils.getAppName(paramActivity));  // 游戏名 : 将在支付页面中显示, 这里设置的话，将会覆盖kaopu_game_config.json中的参数值
        configData.put("screenType", screenType+"");     // 屏幕方向   横屏：1  竖屏：2, 这里设置的话，将会覆盖kaopu_game_config.json中的参数值
...
//    	KaoPu_RoleInfo kaopu = new KaoPu_RoleInfo();
    	KaoPu_RoleInfo.typeId = gameRoleInfo.getTypeId();//上传类型 登录:HY_Constants.ENTER_SERVER 、创角:HY_Constants.CREATE_ROLE、升级:HY_Constants。LEVEL_UP
...
//    	HyLog.d(TAG, kaopu.toString());
        ...
        if (KPSuperSDK.isLogin()) {
            ...
            String id = KaoPu_RoleInfo.roleId;// 游戏方对外用户 ID，即：cp自身用户的userid（不可为空）
            KPSuperSDK.setUserGameRole(mActivity, serviceName, roleName, id, roleLevel);
        	}else{
        	}
        } else {
            Toast.makeText(mActivity, "请先登录", Toast.LENGTH_LONG).show();
